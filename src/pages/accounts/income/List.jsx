...
export default function IncomeistPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await getMyIncome(page, 200);
      if (res.ok) {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        ...
        <Link to="/accounts/income/create" className="button-main">
          New Income
        </Link>
      </div>
      ...
      <div className="flex items-center justify-end">
        <Paggination limit={200} count={count?.data?.totalIncome} nextLink={"/accounts/income"} />
      </div>
    </>
  );
}
