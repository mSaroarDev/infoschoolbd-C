...
import { editCommittee, getACommittee } from "../libs/committeeAPI";
...
export default function EditCommitteeForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setLoading(true)
      const res = await editCommittee(id, values)
      setLoading(false)
      if(res.ok){
        showSuccess("Committee Updated");
        navigate("/user/committee?page=1")
      } else {
        showError("Committee update failed")
      }
    },
  });
