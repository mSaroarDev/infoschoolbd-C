...
import { deleteCommittee, getACommittee } from "../../../libs/committeeAPI";
...
export default function CommitteeProfilePage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await getACommittee(id);
  ...
  const handleDelete = async () => {
    ...
    setLoading(false);
    if (res.ok) {
      showSuccess("Committee profile Deleted");
      navigate("/user/committee?page=1");
    } else {
      ...
      {loading ? (
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <PageHeader
              text="কমিটির প্রোফাইল"
              icon={<CircleUserRound className="w-5 h-5" />}
            />

            <div className="flex items-center gap-1">
              <Link to={`/user/committee/edit/${id}`} className="button-dark">
                এডিট
              </Link>
              <button
