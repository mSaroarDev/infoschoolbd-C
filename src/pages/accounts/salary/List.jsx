...
export default function SalaryistPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await getSalary(page, 200);
      if (res.ok) {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        ...
        <Link to="/accounts/salary/create" className="button-main">
          New Salary Transaction
        </Link>
      </div>
      ...
      <div className="flex items-center justify-end">
        <Paggination
          limit={200}
          count={count?.data?.totalSalary}
          nextLink={"/accounts/salary"}
        />
      </div>
    </>
  );
