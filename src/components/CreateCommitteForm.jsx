...
import { createCommittee } from "../libs/committeeAPI";
...
export default function CreateCommitteeForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setLoading(true)
      const res = await createCommittee(values)

      setLoading(false)
      if(res.ok){
        showSuccess("Committee added");
        navigate("/user/committee?page=1")
      } else {
        showError("Committee adding failed")
      }
    },
  });
      ...
      >
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-12 md:col-span-4 flex flex-col gap-1">
            <label className="text-[15px] italic">অফিসিয়াল পদবী (বাংলায়) <span className="text-red-600">*</span></label>
            <input
              type="text"
              id="designation"
              name="designation"
